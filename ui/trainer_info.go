...
func createTrainerInfo(trainer *data_objects.Trainer) *fyne.Container{ 
    ...
    trainerPicSelectBox := widget.NewSelectEntry(trainerPics)
    ...
    trainerPicSelectBox.OnChanged = func(s string) {
    ...
    itemSelectBox0 := widget.NewSelectEntry(items)
    form.Add(itemLabel0)
    itemValue := trainer.Items[0]
    itemSelectBox0.SetText(itemValue)
    itemSelectBox0.OnChanged = func(s string) {
        if len(s) >= 9 {
    ...
    itemSelectBox1 := widget.NewSelectEntry(items)
    form.Add(itemLabel1)
    itemValue = trainer.Items[1]
    itemSelectBox1.SetText(itemValue)
    itemSelectBox1.OnChanged = func(s string) {
        if len(s) >= 9 {
            if SliceContains(items, s) {
                trainer.Items[1] = s;
            }
    ...
    itemSelectBox2 := widget.NewSelectEntry(items)
    form.Add(itemLabel2)
    itemValue = trainer.Items[2]
    itemSelectBox2.SetText(itemValue)
    itemSelectBox2.OnChanged = func(s string) {
        if len(s) >= 9 {
            if SliceContains(items, s) {
                trainer.Items[2] = s;
            }
    ...
    itemSelectBox3 := widget.NewSelectEntry(items)
    form.Add(itemLabel3)
    itemValue = trainer.Items[3]
    itemSelectBox3.SetText(itemValue)
    itemSelectBox3.OnChanged = func(s string) {
        if len(s) >= 9 {
            if SliceContains(items, s) {
                trainer.Items[3] = s;
            }
